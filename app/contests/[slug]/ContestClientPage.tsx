...
import { Calendar, Share2, Trophy, FileText, AlertTriangle, CheckCircle, XCircle, Clock } from "lucide-react"
...
export default function ContestClientPage({ contest }: ContestClientPageProps) {
  ...
  const processedRules = processText(contest.rules || "No rules specified for this contest.")
  const processedPrizes = processText(contest.prizes || "No prizes specified for this contest.")
  ...
  const deadlineDate = contest.entry_deadline ? new Date(contest.entry_deadline) : endDate
  ...
  const deadlineTimeRemaining =
    timeUntilDeadline > 0
      ? `${hoursUntilDeadline}h ${minutesUntilDeadline}m remaining`
    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="container mx-auto px-4">
          ...
          <div className="lg:col-span-2 opacity-0 animate-fadeIn">
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              ...
              {contest.entry_deadline && (
                <div className="flex items-center">
                  <Clock className="h-5 w-5 text-realty-secondary mr-2" />
                  <span className="text-realty-secondary">
                    Submission Deadline: {new Date(contest.entry_deadline).toLocaleString()}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="prose max-w-none mb-8">
              <p className="text-lg">{processedDescription}</p>
            </div>
              ...
              <div className="text-center mb-6">
                ...
                <div
                  className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium mb-2 ${
                    acceptingSubmissions ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                  }`}
                >
                  <span
                    className={`w-2 h-2 rounded-full mr-2 ${acceptingSubmissions ? "bg-green-500" : "bg-yellow-500"}`}
                  ></span>
                  {acceptingSubmissions ? "Accepting Submissions" : "Not Accepting Submissions"}
                </div>

                {/* Deadline Countdown */}
                {acceptingSubmissions && contest.entry_deadline && (
                ...
                <p className="text-realty-text mt-4">
                  {acceptingSubmissions
                    ? "This contest is currently accepting entries. Scroll down to participate!"
                    : currentTime < startDate
                      ? `Submissions will open on ${startDate.toLocaleString()}`
        ...
        {acceptingSubmissions ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-8">
              ...
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-md">
                <div className="flex items-start">
                  ...
                  <div>
                    <p className="text-blue-800 font-medium">Submission Deadline</p>
                    <p className="text-blue-700">
                      {contest.entry_deadline
                        ? new Date(contest.entry_deadline).toLocaleString()
                        : new Date(contest.endDate).toLocaleString()}
                      {timeUntilDeadline > 0 && (
                        <span className="ml-2 text-sm font-medium">({deadlineTimeRemaining})</span>
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  <label className="flex items-start">
                    ...
                    <span className="text-sm text-realty-text">
                      I agree to the{" "}
                      <Link href="/terms" className="text-realty-secondary hover:text-realty-primary">
                        Terms and Conditions
                      </Link>{" "}
                      and{" "}
                      <Link href="/privacy" className="text-realty-secondary hover:text-realty-primary">
                        Privacy Policy
                      </Link>{" "}
          ...
          <div className="mt-16 opacity-0 animate-fadeIn animate-delay-300">
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              <h2 className="text-2xl font-bold mb-6">
                Submissions {currentTime < startDate ? "Not Yet Open" : "Closed"}
              </h2>

              {currentTime < startDate ? (
                <div className="p-6 bg-yellow-50 rounded-lg inline-block">
                  <Calendar className="h-12 w-12 text-yellow-500 mx-auto mb-4" />
                  <p className="text-lg text-yellow-800 mb-2">This contest will start accepting submissions on:</p>
                  <p className="text-xl font-bold text-yellow-900">{startDate.toLocaleString()}</p>
                </div>
              ) : (
                <div className="p-6 bg-gray-50 rounded-lg inline-block">
                  <Clock className="h-12 w-12 text-gray-500 mx-auto mb-4" />
                  <p className="text-lg text-gray-800 mb-2">The submission deadline has passed.</p>
                  <p className="text-gray-600">Submissions were accepted until:</p>
